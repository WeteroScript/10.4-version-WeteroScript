...
local KICK_STATUS = {
    ...
    hwid_mismatch=true, missing_fields=true,
}
local STATUS_TEXT = {
...
local function updateESPForPlayer(plr)
    ...
    applyHighlightStyle(h, plr); ESP.LastCheck[plr] = tick()
end
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP then
            ...
            if h and h.Parent and (now - last) >= ESP.CheckInterval then
                ESP.LastCheck[plr] = now; applyHighlightStyle(h, plr)
            end
        end
    end
end)
        ...
        if plr ~= LP and isAlive(plr) then
            ...
            if pos then
                ...
                if onScreen and sp.Z > 0 then
                    ...
                    if d <= bestDist and (not Aimbot.VisibleOnly or hasLineOfSight(plr.Character)) then
                        best, bestDist = plr, d
                    end
                end
            end
...
local function aimbotStep()
    ...
    if now < Aimbot.LockReleaseUntil then
        Aimbot.Target = nil; return
    end
    local target = Aimbot.LockedTarget
    if target then
        if not isAlive(target) then
            target = nil; Aimbot.LockedTarget = nil
        elseif Aimbot.VisibleOnly and not hasLineOfSight(target.Character) then
            target = nil; Aimbot.LockedTarget = nil
        end
    end
    if not target then
        target = findNearestTarget(); Aimbot.LockedTarget = target
    end
...
local function hookPlayerForCombat(plr)
    ...
    plr.CharacterRemoving:Connect(function(char)
        removeESPFromChar(char); removeHitboxViewFromChar(char); restoreHitboxForChar(char)
...
local startupSub = new("TextLabel", {
    BackgroundTransparency=1,
    Position=UDim2.new(0, -40, 0, 42),
    Size=UDim2.new(1, -32, 0, 12),
    Text="// v8.6 · t.me//meredioshub",
...
round(keyBox, 7)
local kp = new("UIPadding", keyBox); kp.PaddingLeft = UDim.new(0, 8)
...
do
    local card = makeRow(mainPage, 4, "SERVER HOP", "переброс на другой сервер игры", 58)
    local btn = new("TextButton", {
        Position=UDim2.new(0,12,0,40), Size=UDim2.new(1,-24,0,24),
        BackgroundColor3=Accent.Main, BackgroundTransparency=0.2,
...
do
    local card = makeRow(mainPage, 5, "EMPTY HOP", "сервер с 0 игроков (или минимальным онлайном)", 58)
    local btn = new("TextButton", {
        Position=UDim2.new(0,12,0,40), Size=UDim2.new(1,-24,0,24),
...
do
    ...
    makeSwitch(card, 46, function(v)
        ...
        else restoreAllHitboxes(); if viewState then setView(false) end end
...
do
    ...
    setESPVis = makeSwitch(card, 74, function(v)
        ESP.VisibilityCheck = v; reapplyAllESPStyles()
    ...
    rebuildESPPalette = makeColorPaletteDynamic(card, 142,
        function() return ESP_PALETTE end,
...
do
    local card = makeRow(newPage, 1, "ОБНОВЛЕНИЕ v8.6",
        "t.me//meredioshub — апдейты там", 220)
    local body = new("TextLabel", {
        ...
        Text=table.concat({
            ...
            "• неверный ключ → кик · key is not allowed",
            "• кнопка НАЧАТЬ уменьшена",
            ...
            "• ServerHop · EmptyHop",
            "• Градиент-переливание на карточках",
            "• t.me//meredioshub",
        }, "\n"),
...
local fovLbl = new("TextLabel", {
    BackgroundTransparency=1, Position=UDim2.new(0,18,0,52),
    Size=UDim2.new(1,-36,0,14), Text="FOV RADIUS  (drag — ring comes to front)",
...
makeSlider(fovSliderHost, 6, 40, 400, Aimbot.FOV, function(v)
    Aimbot.FOV = v; updateFOVCircle()
    if _G.MerediosUpdateAimStatus then _G.MerediosUpdateAimStatus() end
end, -84,
function() fovWrap.ZIndex = 6000 end,
function() fovWrap.ZIndex = 0 end)

local fovColLbl = new("TextLabel", {
    BackgroundTransparency=1, Position=UDim2.new(0,18,0,108),
    Size=UDim2.new(1,-36,0,14), Text="FOV COLOR  ·  white gets black outline",
    TextColor3=P.SubText, Font=Enum.Font.GothamBold, TextSize=10,
    TextXAlignment=Enum.TextXAlignment.Left, ZIndex=11,
})
...
local function closeMenu()
    ...
    task.delay(0.30, function()
        ...
        settings.GroupTransparency = 1; settings.BackgroundTransparency = 0.02; settingsStroke.Transparency = 0.12
        aimPanel.GroupTransparency = 1; aimPanel.BackgroundTransparency = 0.02; aimPanelStroke.Transparency = 0.08
        filterPanel.GroupTransparency = 1; filterPanel.BackgroundTransparency = 0.02; filterPanelStroke.Transparency = 0.08
        mBtn.Visible = true
