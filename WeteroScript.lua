--[[
    WeteroScript v11.02 FINAL FIXED v2
    Author: @WeteroScript
    Исправления:
    - Speed/Jump Bot в одной строке с ON/OFF
    - Delete Last Point в Bot Editor
    - Линия от игрока к WP #1
    - Все линии между WP работают
]]
...
local Features = {
    ...
    Bot = {Enabled = false, Speed = 16, SpeedEnabled = false, JumpPower = 50, JumpEnabled = false, JumpOnPoint = false, Waypoints = {}, CurrentWaypointIndex = 0},
    Jerk = {Enabled = false}
}

local connections = {}; local espStorage = {}; local mimicConnections = {}; local botConnections = {}; local botWaypointBeams = {}; local botWaypointParts = {}
local playerToWP1Beam = nil
...
local function stopMimic()
    ...
    if Player.Character then local hum = Player.Character:FindFirstChild("Humanoid"); if hum then hum.WalkSpeed = Features.Speed.Enabled and Features.Speed.Value or 16; hum.JumpPower = Features.Jump.Enabled and Features.Jump.Value or 50 end end
end

local function resetWaypointColors()
    for _, wp in pairs(Features.Bot.Waypoints) do if wp.Part and wp.Part.Parent then wp.Part.Color = Color3.fromRGB(60, 100, 255); if wp.Beam and wp.Beam.Parent then wp.Beam.Color = ColorSequence.new(Color3.fromRGB(60, 100, 255)) end; for _, child in pairs(wp.Part:GetChildren()) do if child:IsA("PointLight") then child.Color = Color3.fromRGB(60, 100, 255) end end end end
end
...
local function attachLastBeamToPlayer()
    ...
    if lastWp and lastWp.Beam and lastWp.Beam.Parent and lastWp.Beam.Attachment0 then lastWp.Beam.Attachment0.Parent = root end
end

local function detachLastBeamFromPlayer()
    if #Features.Bot.Waypoints == 0 then return end
    local lastWp = Features.Bot.Waypoints[#Features.Bot.Waypoints]
    if lastWp and lastWp.Beam and lastWp.Beam.Parent and lastWp.Beam.Attachment0 then lastWp.Beam.Attachment0.Parent = lastWp.Part end
end

local function createPlayerToWP1Beam()
    if playerToWP1Beam and playerToWP1Beam.Parent then playerToWP1Beam:Destroy() end
    playerToWP1Beam = nil
    if #Features.Bot.Waypoints == 0 then return end
    local root = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    local wp1 = Features.Bot.Waypoints[1]
    if not wp1 or not wp1.Part or not wp1.Part.Parent then return end
    local beam = Instance.new("Beam")
    beam.Attachment0 = Instance.new("Attachment"); beam.Attachment0.Parent = root
    beam.Attachment1 = Instance.new("Attachment"); beam.Attachment1.Parent = wp1.Part
    ...
    beam.Parent = wp1.Part
    playerToWP1Beam = beam
end

local function removePlayerToWP1Beam()
    if playerToWP1Beam and playerToWP1Beam.Parent then playerToWP1Beam:Destroy() end
    playerToWP1Beam = nil
end

local function updateWaypointBeams()
    for i = 1, #Features.Bot.Waypoints do
        local wp = Features.Bot.Waypoints[i]
        if wp and wp.Beam and wp.Beam.Parent then
            if i < #Features.Bot.Waypoints then
                local nextWp = Features.Bot.Waypoints[i + 1]
                if nextWp and nextWp.Part and nextWp.Part.Parent then
                    if wp.Beam.Attachment0 then wp.Beam.Attachment0.Parent = wp.Part end
                    if wp.Beam.Attachment1 then wp.Beam.Attachment1.Parent = nextWp.Part end
                end
            end
        end
    end
...
local function stopBot()
    ...
    if Player.Character then local hum = Player.Character:FindFirstChild("Humanoid"); if hum then hum.WalkSpeed = Features.Speed.Enabled and Features.Speed.Value or 16; hum.JumpPower = Features.Jump.Enabled and Features.Jump.Value or 50 end end
    resetWaypointColors(); attachLastBeamToPlayer(); removePlayerToWP1Beam()
end

local function clearWaypointBeams()
    removePlayerToWP1Beam()
    for _, wp in pairs(Features.Bot.Waypoints) do if wp.Beam and wp.Beam.Parent then wp.Beam:Destroy() end; if wp.Part and wp.Part.Parent then wp.Part:Destroy() end end
    Features.Bot.Waypoints = {}; botWaypointBeams = {}; botWaypointParts = {}
end
...
local function stopAllFunctions()
    ...
    connections = {}; Features.WalkFling = false; Features.Jerk.Enabled = false
    if autoJumpHeartbeat then autoJumpHeartbeat:Disconnect(); autoJumpHeartbeat = nil end
    if jerkHeartbeat then jerkHeartbeat:Disconnect(); jerkHeartbeat = nil end
...
local function startAutoJump()
    ...
    autoJumpHeartbeat = RunService.Heartbeat:Connect(function()
        if not autoJumpEnabled then return end; local char = Player.Character; if not char then return end
...
-- ==================== JERK ====================
local jerkEnabled = false; local jerkHeartbeat = nil
local function startJerk()
    if jerkHeartbeat then jerkHeartbeat:Disconnect() end
    jerkHeartbeat = RunService.Heartbeat:Connect(function()
        if not jerkEnabled then return end; local char = Player.Character; if not char then return end
        local root = char:FindFirstChild("HumanoidRootPart"); local hum = char:FindFirstChild("Humanoid")
        if root and hum and hum.Health > 0 then root.Velocity = Vector3.new(math.random(-200, 200), math.random(-200, 200), math.random(-200, 200)) end
    end)
end
...
local MimicMenuWindow = Instance.new("Frame"); MimicMenuWindow.Size = UDim2.new(0, 220, 0, 240); MimicMenuWindow.Position = UDim2.new(0.7, -110, 0.3, -120)
...
local function applyFollowOffset()
    if Features.Mimic.FollowOffsetEnabled then Features.Mimic.FollowOffset = tonumber(MMFOInput.Text) or 5; if Features.Mimic.Enabled and Features.Mimic.Target then teleportToTarget() end else Features.Mimic.FollowOffset = 0 end
end
...
local function applyFollowDelay()
    if Features.Mimic.FollowDelayEnabled then Features.Mimic.PositionDelay = tonumber(MMFDInput.Text) or 0 else Features.Mimic.PositionDelay = 0 end
end
...
local flySpeed = 50; local flyDirection = Vector3.new(); local flyActive = false; local inputStates = {Up = false, Down = false, Left = false, Right = false, Space = false, Ctrl = false}
...
addToggle(page3, layout3, "Jerk", function(e)
    jerkEnabled = e; Features.Jerk.Enabled = e
    if e then startJerk() else if jerkHeartbeat then jerkHeartbeat:Disconnect(); jerkHeartbeat = nil end end
end)
...
addToggle(page4, layout4, "Start Mimic", function(e)
    ...
    if e then
        ...
        if myRoot then
            ...
            myRoot.CFrame = CFrame.new(tc.Position + (-tc.LookVector * 5), tc.Position)
        end
...
-- Jump Power Bot
local botJumpContainer = Instance.new("Frame"); botJumpContainer.Size = UDim2.new(1, 0, 0, 34); botJumpContainer.BackgroundColor3 = Color3.fromRGB(32, 32, 46)
botJumpContainer.BorderSizePixel = 0; botJumpContainer.Parent = page5; addCorner(botJumpContainer, 5); addStroke(botJumpContainer, 1, Color3.fromRGB(50, 50, 70))
local botJumpLabel = Instance.new("TextLabel"); botJumpLabel.Size = UDim2.new(0, 70, 1, 0); botJumpLabel.Position = UDim2.new(0, 8, 0, 0)
botJumpLabel.BackgroundTransparency = 1; botJumpLabel.Text = "Jump Power"; botJumpLabel.TextColor3 = Color3.new(1, 1, 1); botJumpLabel.Font = Enum.Font.Gotham; botJumpLabel.TextSize = 10; botJumpLabel.TextXAlignment = Enum.TextXAlignment.Left; botJumpLabel.Parent = botJumpContainer
local botJumpInputBg = Instance.new("Frame"); botJumpInputBg.Size = UDim2.new(0, 40, 0, 20); botJumpInputBg.Position = UDim2.new(0, 80, 0.5, -10); botJumpInputBg.BackgroundColor3 = Color3.fromRGB(22, 22, 34); botJumpInputBg.BorderSizePixel = 0; botJumpInputBg.Parent = botJumpContainer; addCorner(botJumpInputBg, 3)
local botJumpInput = Instance.new("TextBox"); botJumpInput.Size = UDim2.new(1, -6, 1, 0); botJumpInput.Position = UDim2.new(0, 3, 0, 0); botJumpInput.BackgroundTransparency = 1
botJumpInput.PlaceholderText = "50"; botJumpInput.PlaceholderColor3 = Color3.fromRGB(100, 100, 120); botJumpInput.Text = "50"; botJumpInput.TextColor3 = Color3.new(1, 1, 1)
botJumpInput.Font = Enum.Font.Gotham; botJumpInput.TextSize = 10; botJumpInput.ZIndex = 10; botJumpInput.Parent = botJumpInputBg
local botJumpToggleBg = Instance.new("Frame"); botJumpToggleBg.Size = UDim2.new(0, 32, 0, 16); botJumpToggleBg.Position = UDim2.new(1, -42, 0.5, -8); botJumpToggleBg.BackgroundColor3 = Color3.fromRGB(22, 22, 34); botJumpToggleBg.BorderSizePixel = 0; botJumpToggleBg.Parent = botJumpContainer; addCorner(botJumpToggleBg, 1)
local botJumpDot = Instance.new("Frame"); botJumpDot.Size = UDim2.new(0, 12, 0, 12); botJumpDot.Position = UDim2.new(0, 2, 0.5, -6); botJumpDot.BackgroundColor3 = Color3.fromRGB(180, 180, 200); botJumpDot.BorderSizePixel = 0; botJumpDot.Parent = botJumpToggleBg; addCorner(botJumpDot, 1)
local botJumpClick = Instance.new("TextButton"); botJumpClick.Size = UDim2.new(0, 40, 1, 0); botJumpClick.Position = UDim2.new(1, -44, 0, 0); botJumpClick.BackgroundTransparency = 1; botJumpClick.Text = ""; botJumpClick.ZIndex = 5; botJumpClick.Parent = botJumpContainer
botJumpClick.Activated:Connect(function()
    Features.Bot.JumpEnabled = not Features.Bot.JumpEnabled
    botJumpToggleBg.BackgroundColor3 = Features.Bot.JumpEnabled and Color3.fromRGB(60, 140, 60) or Color3.fromRGB(22, 22, 34)
    botJumpDot:TweenPosition(UDim2.new(0, Features.Bot.JumpEnabled and 18 or 2, 0.5, -6), "Out", "Quad", 0.15)
    botJumpDot.BackgroundColor3 = Features.Bot.JumpEnabled and Color3.new(1, 1, 1) or Color3.fromRGB(180, 180, 200)
    if Features.Bot.JumpEnabled then
        Features.Bot.JumpPower = tonumber(botJumpInput.Text) or 50
        if Features.Bot.Enabled and Player.Character then
            local hum = Player.Character:FindFirstChild("Humanoid")
            if hum then hum.JumpPower = Features.Bot.JumpPower end
        end
    else
        if Features.Bot.Enabled and Player.Character then
            local hum = Player.Character:FindFirstChild("Humanoid")
...
botJumpInput.FocusLost:Connect(function()
    if Features.Bot.JumpEnabled then
        Features.Bot.JumpPower = tonumber(botJumpInput.Text) or 50
        if Features.Bot.Enabled and Player.Character then
            local hum = Player.Character:FindFirstChild("Humanoid")
...
local BotEditorWindow = Instance.new("Frame"); BotEditorWindow.Size = UDim2.new(0, 280, 0, 250); BotEditorWindow.Position = UDim2.new(0.7, -140, 0.3, -125); BotEditorWindow.BackgroundColor3 = Color3.fromRGB(10, 10, 18); BotEditorWindow.BorderSizePixel = 0; BotEditorWindow.Visible = false; BotEditorWindow.Parent = BotEditorGui; addCorner(BotEditorWindow, 12); addStroke(BotEditorWindow, 2, Color3.fromRGB(60, 100, 255))
...
local function highlightWaypoint(index)
    ...
    if index > 0 and index <= #Features.Bot.Waypoints then
        local wp = Features.Bot.Waypoints[index]; if wp.Part and wp.Part.Parent then wp.Part.Color = Color3.fromRGB(0, 255, 0); if wp.Beam and wp.Beam.Parent then wp.Beam.Color = ColorSequence.new(Color3.fromRGB(0, 255, 0)) end; for _, child in pairs(wp.Part:GetChildren()) do if child:IsA("PointLight") then child.Color = Color3.fromRGB(0, 255, 0) end end end
    end
end
...
addBEBtn("📍 Create Waypoint", function()
    ...
    local beam = Instance.new("Beam")
    beam.Attachment0 = Instance.new("Attachment"); beam.Attachment0.Parent = part
    ...
    if n > 1 then
        ...
        if prevWp and prevWp.Part and prevWp.Part.Parent then
            beam.Attachment1.Parent = prevWp.Part
        end
    end
    updateWaypointBeams()
    createPlayerToWP1Beam()
end)
...
addBEBtn("🗑 Delete Last Point", function()
    ...
    if lastWp then
        ...
        if lastWp.Beam and lastWp.Beam.Parent then lastWp.Beam:Destroy() end
    end
    table.remove(Features.Bot.Waypoints, #Features.Bot.Waypoints)
    if #botWaypointBeams > 0 then table.remove(botWaypointBeams, #botWaypointBeams) end
...
botToggleBtn.Activated:Connect(function()
    ...
    if Features.Bot.Enabled then
        ...
        if Player.Character then
            ...
            if hum then
                ...
                hum.JumpPower = Features.Bot.JumpEnabled and Features.Bot.JumpPower or 50
            end
        end
        botToggleBtn.Text = "⏸ Stop Bot"; botToggleStroke.Color = Color3.fromRGB(60, 140, 60)
        detachLastBeamFromPlayer(); removePlayerToWP1Beam()
        ...
        botConnections.Walk = RunService.Heartbeat:Connect(function()
            ...
            if not wp or not wp.Part or not wp.Part.Parent then
                local found = false
                for _ = 1, #Features.Bot.Waypoints do currentIdx = currentIdx + 1; if currentIdx > #Features.Bot.Waypoints then currentIdx = 1 end; local cw = Features.Bot.Waypoints[currentIdx]; if cw and cw.Part and cw.Part.Parent then wp = cw; found = true; break end end
            ...
            if distance > 4 then
                ...
                hum.WalkSpeed = Features.Bot.SpeedEnabled and Features.Bot.Speed or 16
                local rayOrigin = root.Position + Vector3.new(0, 1, 0); local raycastResult = Workspace:Raycast(rayOrigin, direction.Unit * 3)
    ...
    else botToggleBtn.Text = "▶ Start Bot"; botToggleStroke.Color = Color3.fromRGB(40, 40, 60); stopBot(); createPlayerToWP1Beam() end
end)
...
addToggle(page5, layout5, "Edit Bot (Open Editor)", function(e) BotEditorWindow.Visible = e; BotButton.Visible = e; if not e then stopBot() else createPlayerToWP1Beam() end end)
...
Player.CharacterAdded:Connect(function(char)
    ...
    if jerkEnabled then startJerk() end
    if not Features.Bot.Enabled then createPlayerToWP1Beam() end
end)

-- ==================== SELECT FIRST TAB ====================
TabPages[1].Page.Visible = true; TabBtns[1].BackgroundColor3 = Color3.fromRGB(60, 60, 80); TabBtns[1].TextColor3 = Color3.new(1, 1, 1)

-- Создаём линию от игрока к WP #1 при старте
createPlayerToWP1Beam()
