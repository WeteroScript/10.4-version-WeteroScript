--[[
    WeteroScript v11.03 FINAL FIXED v13 - FULLY REPAIRED
    Author: @WeteroScript
    FIXED: All 12 critical errors + memory leaks + injection ready
]]
...
-- FIXED: createBeamBetweenParts with duplicate prevention
local function createBeamBetweenParts(partA, partB)
    if not partA or not partB or not partA.Parent or not partB.Parent then return nil end
    
    -- Remove existing beams between these parts
    for _, child in pairs(partA:GetChildren()) do
        if child:IsA("Beam") and child.Attachment1 and child.Attachment1.Parent == partB then
            child:Destroy()
        end
    end
...
local function addToggleWithInput(page, layout, text, placeholder, defaultVal, toggleCallback, inputCallback)
    ...
    click.Activated:Connect(function()
        ...
        local tween = TweenService:Create(dot, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0, enabled and 18 or 2, 0.5, -6)})
        tween:Play()
...
local connections = {}; local espStorage = {}; local mimicConnections = {}; local botConnections = {}; local lastWPToPlayerBeam = nil
local flyActive = false; local flyHeartbeat
local autoJumpEnabled = false; local autoJumpHeartbeat = nil

-- ==================== BEAM FUNCTIONS (FIXED) ====================
...
-- FIXED: clearWaypointBeams with proper cleanup
...
-- FIXED: stopAllFunctions with complete cleanup
...
-- ==================== ESP FUNCTIONS (FIXED FOR NEW PLAYERS + MEMORY LEAKS) ====================
local function refreshESP()
    -- Clean up existing ESP data
    ...
    -- Remove old connections to prevent duplicates
    ...
    -- Listen for CharacterAdded on existing players
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= Player then
...
FlyToggleClick.Activated:Connect(function()
    ...
    local tween = TweenService:Create(FlyToggleDot, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0, flyActive and 18 or 2, 0.5, -6)})
    tween:Play()
...
local function startMobileFly()
    ...
    if FlyToggleDot then 
        local tween = TweenService:Create(FlyToggleDot, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0, 18, 0.5, -6)})
        tween:Play()
...
addToggleWithInput(page2, layout2, "Hitbox", "5", "5", function(e) 
...
end, function(v) Features.Hitbox.Size = tonumber(v) or 5 end)
...
botSpeedClick.Activated:Connect(function()
    ...
    local tween = TweenService:Create(botSpeedDot, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0, Features.Bot.SpeedEnabled and 18 or 2, 0.5, -6)})
    tween:Play()
...
-- ==================== BOT EDITOR (WITH ORIGINAL LINES - FIXED) ====================
...
addBEBtn("📍 Create Waypoint", function()
    ...
    -- FIXED: Create beam to first waypoint when only one exists
    if #Features.Bot.Waypoints == 1 and not Features.Bot.Enabled then
        createLastWPToPlayerBeam()
    end
end)
...
-- ==================== END OF SCRIPT ====================
print("✅ WeteroScript v11.03 FINAL FIXED v13 - Successfully injected and running!")
