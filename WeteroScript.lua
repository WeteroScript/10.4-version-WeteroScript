--[[
    WeteroScript v11.02 FINAL FIXED v3
    Author: @WeteroScript
    Логика линий:
    - WP #1 создана → синяя линия WP #1 → персонаж (следует)
    - WP #2 создана → линия WP #1 → WP #2 (фикс), новая линия WP #2 → персонаж
    - Start Bot → линия исчезает
    - Stop Bot → линия от последней WP к персонажу
]]
...
-- Удаляет луч от последней WP к игроку
local function removeLastWPToPlayerBeam()
    if lastWPToPlayerBeam and lastWPToPlayerBeam.Parent then lastWPToPlayerBeam:Destroy() end
    lastWPToPlayerBeam = nil
end

-- Создаёт луч от последней WP к игроку
...
-- Фиксирует луч от последней WP к предыдущей (когда создаётся новая WP)
local function fixLastBeamToPreviousWP()
    ...
    if prevWp and prevWp.Part and prevWp.Part.Parent and lastWp and lastWp.Part and lastWp.Part.Parent then
        removeLastWPToPlayerBeam()
        -- Создаём постоянный луч между prevWp и lastWp
        local beam = Instance.new("Beam")
...
addBEBtn("📍 Create Waypoint", function()
    ...
    -- Фиксируем предыдущий луч, если есть
    if n > 1 then
        fixLastBeamToPreviousWP()
    end

    table.insert(Features.Bot.Waypoints, {Part = part, Position = pos})
    table.insert(botWaypointParts, part)

    -- Создаём новый луч от последней WP к игроку
    createLastWPToPlayerBeam()
end)
...
addBEBtn("🗑 Delete Last Point", function()
    ...
    -- Удаляем луч к игроку и создаём заново от новой последней WP
    removeLastWPToPlayerBeam()
