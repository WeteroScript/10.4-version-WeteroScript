--[[
    WeteroScript v11.02 FINAL FIXED v11
    Author: @WeteroScript
    ADDED: R6 Animations Tab + 10 Custom Animations
]]
...
local Window = Instance.new("Frame"); Window.Size = UDim2.new(0, 620, 0, 480); Window.Position = UDim2.new(0.5, -310, 0.5, -240)
...
local Features = {
    ...
    R6Anim = {Enabled = false, CurrentAnim = nil, CurrentTrack = nil, Animator = nil}
}
...
-- СЛОВАРЬ АНИМАЦИЙ (10 штук)
local R6_ANIMATIONS = {
    -- Базовые животные
    Dog = {
        name = "🐕 DOG",
        desc = "Бег на четвереньках, язык наружу",
        animId = "rbxassetid://1844205409", -- Анимация собаки (замени на свой ID)
    ...
    Snake = {
        name = "🐍 SNAKE",
        desc = "Ползание по земле, извиваясь",
        animId = "rbxassetid://1314880486", -- Анимация змеи (замени на свой ID)
        priority = Enum.AnimationPriority.Action,
        loop = true,
        speed = 1.2,
        walkSpeed = 12
    ...
    Cat = {
        name = "🐱 CAT",
        desc = "Кошачья грация, мягкие шаги",
        animId = "rbxassetid://1887714601", -- Анимация кошки (замени на свой ID)
        priority = Enum.AnimationPriority.Action,
        loop = true,
        speed = 1.3,
        walkSpeed = 18
    ...
    Spider = {
        name = "🕷️ SPIDER",
        desc = "Паук, 8 лап, стена",
        animId = "rbxassetid://1314880486",
        priority = Enum.AnimationPriority.Action,
        loop = true,
        speed = 1.1,
        walkSpeed = 14
    ...
    Robot = {
        name = "🤖 ROBOT",
        desc = "Робот, механические движения",
        animId = "rbxassetid://1314880486",
        priority = Enum.AnimationPriority.Action,
        loop = true,
        speed = 0.7,
...
local function stopR6Animation()
    if Features.R6Anim.CurrentTrack then
        pcall(function() Features.R6Anim.CurrentTrack:Stop() end)
        Features.R6Anim.CurrentTrack = nil
    end
    Features.R6Anim.CurrentAnim = nil
    Features.R6Anim.Enabled = false
    
    -- Восстановление стандартной скорости
...
local function playR6Animation(animKey)
    ...
    local animData = R6_ANIMATIONS[animKey]
    if not animData then return end
    
    -- Получаем или создаем Animator
    local animator = hum:FindFirstChild("Animator")
    if not animator then
        animator = Instance.new("Animator")
        animator.Parent = hum
    end
    Features.R6Anim.Animator = animator
    
    -- Создаем анимацию
    local animation = Instance.new("Animation")
    animation.AnimationId = animData.animId
    ...
    -- Сохраняем состояние
    Features.R6Anim.CurrentTrack = track
    Features.R6Anim.CurrentAnim = animKey
    Features.R6Anim.Enabled = true
    
    -- Меняем скорость ходьбы
    if animData.walkSpeed then
        hum.WalkSpeed = animData.walkSpeed
    end
    
    -- Дополнительные эффекты
    if animKey == "Frog" and animData.autoJump then
        -- Лягушка автоматически прыгает
        local frogJumpConn
        frogJumpConn = RunService.Heartbeat:Connect(function()
            if not Features.R6Anim.Enabled or Features.R6Anim.CurrentAnim ~= "Frog" then
                if frogJumpConn then frogJumpConn:Disconnect() end
                return
            end
        ...
        table.insert(connections, frogJumpConn)
    end
    
    -- Уведомление
    print("🎬 Анимация включена: " .. animData.name)
end

-- Обновление отображения текущей анимации
local currentAnimLabel = nil
...
currentAnimLabel = Instance.new("TextLabel")
...
stopAnimBtn.Activated:Connect(function()
    stopR6Animation()
    if currentAnimLabel then currentAnimLabel.Text = "🎬 Current: None" end
end)

-- Создание кнопок для каждой анимации
local animKeys = {"Dog", "Snake", "Cat", "Spider", "Bird", "Zombie", "Ninja", "Dino", "Frog", "Robot"}

for _, key in ipairs(animKeys) do
    local animData = R6_ANIMATIONS[key]
    ...
    playBtn.Activated:Connect(function()
        ...
        if currentAnimLabel then 
            currentAnimLabel.Text = "🎬 Current: " .. animData.name
        end
    end)
...
Player.CharacterAdded:Connect(function(char)
    ...
    if Features.R6Anim.Enabled and Features.R6Anim.CurrentAnim then
        task.wait(0.2)
        playR6Animation(Features.R6Anim.CurrentAnim)
    end
end)
