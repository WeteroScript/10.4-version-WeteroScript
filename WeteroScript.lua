--[[
    WeteroScript v11.02 FINAL FIXED v5
    Author: @WeteroScript
]]
...
GotoTargetBtn.Activated:Connect(function() teleportToTarget() end)
...
addBEBtn("🗑 Delete Last Point", function()
    ...
    -- Удаляем луч от предпоследней WP к удалённой (если есть)
    ...
    if Features.Bot.Enabled then
        ...
        botConnections.Walk = RunService.Heartbeat:Connect(function()
            ...
            if distance > 4 then
                ...
                if direction.Magnitude > 0.1 then root.CFrame = CFrame.new(root.Position, root.Position + direction.Unit); local moveDistance = math.min((Features.Bot.SpeedEnabled and Features.Bot.Speed or 16) * 0.05, distance); root.CFrame = root.CFrame + (direction.Unit * moveDistance); hum:Move(direction.Unit, false) end
