--[[
    ...
    Author: @WeteroScript
    FIX: Bot movement from v1.00 + Mimic menu hide fix
]]
...
local function hideMimicMenu()
    pcall(function() if MimicMenuBtn then MimicMenuBtn.Visible = false end end)
    pcall(function() if MimicMenuWindow then MimicMenuWindow.Visible = false end end)
end
...
-- ==================== BOT MOVEMENT (ВЗЯТО ИЗ WeteroScript v1.00) ====================
...
-- СПОСОБ ХОДЬБЫ ИЗ ВТОРОГО СКРИПТА (WeteroScript v1.00)
local function startBotMovement()
    ...
    botConnections.Walk = RunService.Heartbeat:Connect(function()
        ...
        -- ПОВОРОТ К ТОЧКЕ (как во втором скрипте)
        ...
        if distance > 4 then
            ...
            -- ДВИЖЕНИЕ (как во втором скрипте)
...
-- ФИКС: При выключении Start Mimic скрываем MimicMenuBtn и MimicMenuWindow
...
botToggleBtn.Activated:Connect(function()
    ...
    if Features.Bot.Enabled then
        ...
        -- ИСПОЛЬЗУЕМ НОВУЮ ФУНКЦИЮ ХОДЬБЫ ИЗ ВТОРОГО СКРИПТА
        startBotMovement()
    else
