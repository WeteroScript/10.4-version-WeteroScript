--[[
    WeteroScript v11.02 FINAL FIXED v6
    Author: @WeteroScript
]]
...
local flyActive = false; local flyHeartbeat; local flyControlGui
...
-- Goto Target (телепорт + переключение мимика)
...
GotoTargetBtn.Activated:Connect(function()
    ...
    if Features.Mimic.Enabled then
        -- Перезапускаем мимик на ту же цель
        if mimicConnections.Main then mimicConnections.Main:Disconnect(); mimicConnections.Main = nil end
        startMimicFollow()
    end
end)
...
FlySpeedInput.FocusLost:Connect(function()
    Features.Fly.Speed = math.clamp(tonumber(FlySpeedInput.Text) or 50, 10, 500)
...
function stopMobileFly()
    ...
    FlyToggleBg.BackgroundColor3 = Color3.fromRGB(22, 22, 34)
    FlyToggleDot.Position = UDim2.new(0, 2, 0.5, -6)
    FlyToggleDot.BackgroundColor3 = Color3.fromRGB(180, 180, 200)
