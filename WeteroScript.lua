--[[
    WeteroScript v11.02 FINAL FIXED v9
    Author: @WeteroScript
]]
...
local function fixLastBeamToPreviousWP()
    ...
    if prevWp and prevWp.Part and prevWp.Part.Parent and lastWp and lastWp.Part and lastWp.Part.Parent then
        -- Удаляем временный луч от предыдущей WP к игроку
        if prevWp.TempBeam and prevWp.TempBeam.Parent then
            destroyBeam(prevWp.TempBeam)
            prevWp.TempBeam = nil
        end
        removeLastWPToPlayerBeam()
...
local function clearWaypointBeams()
    ...
    for _, wp in pairs(Features.Bot.Waypoints) do
        ...
        if wp.TempBeam then destroyBeam(wp.TempBeam) end
...
-- Goto Target (телепорт + все функции мимика на новой цели)
...
GotoTargetBtn.Activated:Connect(function()
    if not Features.Mimic.Target or not Features.Mimic.Target.Parent then return end
    -- Обновляем цель и телепортируемся
    teleportToTarget()
    -- Если мимик включен, перезапускаем все функции на новой цели
...
-- Mimic Menu Button (скрывается/появляется как у Bot Editor)
...
-- ==================== FLY CONTROL GUI (БЕЗ ЧЁРНОЙ КНОПКИ) ====================
...
local function startMobileFly()
    ...
    FlyGui.Enabled = true; DPad.Visible = true
...
addToggle(page3, layout3, "Fly (Mobile)", function(e)
    ...
    if e then
        FlyControlWindow.Visible = true
        DPad.Visible = true
        FlyGui.Enabled = true
        FlySpeedInput.Text = tostring(Features.Fly.Speed)
    else
        stopMobileFly()
        FlyControlWindow.Visible = false
        DPad.Visible = false
...
addBEBtn("📍 Create Waypoint", function()
    ...
    -- Сохраняем TempBeam перед фиксацией
    if n > 1 then
        fixLastBeamToPreviousWP()
    end

    local wpData = {Part = part, Position = pos}
    table.insert(Features.Bot.Waypoints, wpData)
    ...
    if root2 then beam.Attachment1.Parent = root2 end
    beam.Width0 = 0.2; beam.Width1 = 0.2
    beam.Color = ColorSequence.new(Color3.fromRGB(60, 100, 255))
    beam.Transparency = NumberSequence.new(0.3)
    beam.Parent = part
    lastWPToPlayerBeam = beam
    wpData.TempBeam = beam
    registerBeam(beam)
end)
...
addBEBtn("🗑 Delete Last Point", function()
    ...
    if lastWp then
        ...
        if lastWp.TempBeam then destroyBeam(lastWp.TempBeam) end
