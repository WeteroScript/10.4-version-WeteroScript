...
AdTitle.BackgroundTransparency = 1; AdTitle.Text = "WeteroScript v13.1"; AdTitle.TextColor3 = Color3.new(1, 1, 1)
...
-- MAIN GUI - УЗКОЕ, НИЖЕ, ДВИГАЕМОЕ
local Gui = Instance.new("ScreenGui"); Gui.Name = "WeteroScript"; Gui.Parent = CoreGui; Gui.ResetOnSpawn = false
local Window = Instance.new("Frame"); Window.Size = UDim2.new(0, 480, 0, 520); Window.Position = UDim2.new(0.5, -240, 0.6, -260)
...
Title.BackgroundTransparency = 1; Title.Text = "⚡ WeteroScript v13.1"; Title.TextColor3 = Color3.fromRGB(200, 200, 255)
...
local function sendChatMessage(msg)
    ...
    -- Поиск всех возможных remote для отправки сообщения
    local success = false
    
    -- Способ 1: SayMessageRequest
    ...
    -- Способ 2: DefaultChatSystemChatEvents
    local defaultChat = ReplicatedStorage:FindFirstChild("DefaultChatSystemChatEvents")
    if defaultChat and defaultChat:FindFirstChild("SayMessageRequest") then
        pcall(function() defaultChat.SayMessageRequest:FireServer(msg, "All") end)
        success = true
    end
    
    -- Способ 3: Chat Remote
    ...
    -- Способ 4: Через TextChatService (новые игры)
    ...
    if success then
        ...
        addChatMessage("ERROR", "Could not send message - unknown chat system", true)
    end
end

local function hookChat()
    -- Способ 1: ChatServiceRunner
    local success, chatService = pcall(function()
        return require(ReplicatedStorage:FindFirstChild("ChatServiceRunner")).Client
    end)
    ...
    -- Способ 2: SayMessageRequest
    local sayMessage = ReplicatedStorage:FindFirstChild("SayMessageRequest")
    if sayMessage and sayMessage.OnClientEvent then
    ...
    -- Способ 3: DefaultChatSystemChatEvents
    local defaultChat = ReplicatedStorage:FindFirstChild("DefaultChatSystemChatEvents")
    if defaultChat and defaultChat.OnClientEvent then
        defaultChat.OnClientEvent:Connect(function(data)
            if data and data.FromSpeaker and data.Message then
                addChatMessage(data.FromSpeaker, data.Message, false)
            end
        end)
    ...
    -- Способ 4: TextChatService (новые игры)
    local textChatService = game:GetService("TextChatService")
    if textChatService then
        local onMessage = textChatService.OnMessageReceived or textChatService.MessageReceived
        if onMessage then
            onMessage:Connect(function(message)
                if message and message.Text and message.FromSpeaker then
                    addChatMessage(message.FromSpeaker.Name or "Unknown", message.Text, false)
                end
            end)
        end
    end
    
    addChatMessage("SYSTEM", "Chat system hooked! Other players' messages will appear here.", true)
end
...
FlySpeedInput.FocusLost:Connect(function()
    ...
    if val then
        Features.Fly.Speed = math.clamp(val, 10, 500)
        FlySpeedInput.Text = tostring(Features.Fly.Speed)
    else
        FlySpeedInput.Text = tostring(Features.Fly.Speed)
    end
    if FlySpeedText then FlySpeedText.Text = tostring(Features.Fly.Speed) end
end)

-- MOBILE FLY GUI
local FlyGui = Instance.new("ScreenGui"); FlyGui.Name = "FlyGui"; FlyGui.Parent = CoreGui; FlyGui.ResetOnSpawn = false; FlyGui.Enabled = false
...
local function createDPadButton(name, position, text)
    local btn = Instance.new("TextButton"); btn.Name = name; btn.Size = UDim2.new(0, 45, 0, 45); btn.Position = position; btn.BackgroundColor3 = Color3.fromRGB(30, 30, 40); btn.Text = text; btn.TextColor3 = Color3.new(1, 1, 1); btn.Font = Enum.Font.GothamBold; btn.TextSize = 18; btn.BorderSizePixel = 0; btn.AutoButtonColor = false; btn.Parent = DPad; addCorner(btn, 8); addStroke(btn, 1, Color3.fromRGB(60, 100, 255)); return btn
end
local UpBtn = createDPadButton("Up", UDim2.new(0.5, -22, 0, 10), "▲")
local DownBtn = createDPadButton("Down", UDim2.new(0.5, -22, 0, 105), "▼")
local LeftBtn = createDPadButton("Left", UDim2.new(0, 10, 0.5, -22), "◀")
local RightBtn = createDPadButton("Right", UDim2.new(1, -55, 0.5, -22), "▶")
local flyDirection = Vector3.new()
local inputStates = {Up = false, Down = false, Left = false, Right = false, Space = false}
local function updateFlyVelocity() 
    if not flyActive then return end
    local cam = workspace.CurrentCamera
    if not cam then return end
    local dir = Vector3.new() 
    if inputStates.Up then dir = dir + cam.CFrame.LookVector end 
    if inputStates.Down then dir = dir - cam.CFrame.LookVector end 
    if inputStates.Left then dir = dir - cam.CFrame.RightVector end 
    if inputStates.Right then dir = dir + cam.CFrame.RightVector end 
...
local function connectDPadButton(btn, key)
    ...
    btn.MouseButton1Up:Connect(function() inputStates[key] = false; btn.BackgroundColor3 = Color3.fromRGB(30, 30, 40); updateFlyVelocity() end)
    btn.MouseLeave:Connect(function() if inputStates[key] then inputStates[key] = false; btn.BackgroundColor3 = Color3.fromRGB(30, 30, 40); updateFlyVelocity() end end)
end
...
FlyButton.Activated:Connect(function() 
    ...
    if now - lastTap < 0.3 then 
        ...
        updateFlyVelocity()
        FlyButton.BackgroundColor3 = inputStates.Space and Color3.fromRGB(60, 100, 255) or Color3.fromRGB(0, 0, 0)
    end
    lastTap = now 
end)
...
local function startMobileFly()
    ...
    local bg = Instance.new("BodyGyro"); bg.MaxTorque = Vector3.new(9e9, 9e9, 9e9); bg.Parent = root
    FlyGui.Enabled = true
    if FlyToggleBg then FlyToggleBg.BackgroundColor3 = Color3.fromRGB(60, 140, 60) end
    if FlyToggleDot then FlyToggleDot.Position = UDim2.new(0, 18, 0.5, -6); FlyToggleDot.BackgroundColor3 = Color3.new(1, 1, 1) end
    flyHeartbeat = RunService.Heartbeat:Connect(function() 
        if not root or not root.Parent or not flyActive then return end
        updateFlyVelocity()
        local vel = flyDirection.Magnitude > 0 and flyDirection.Unit * Features.Fly.Speed or Vector3.new()
        bv.Velocity = vel
        local cam = workspace.CurrentCamera
...
addButton(page2, layout2, "🎨 Open Color Palette", function() if ColorPickerWindow then ColorPickerWindow.Visible = true end end)
    ...
    function(e) 
        ...
        if e then
            ...
            if connections.HitboxExtras then
                for _, c in pairs(connections.HitboxExtras) do pcall(function() c:Disconnect() end) end
    ...
    if e then
        ...
        if cam then
            ...
            connections.Freezecam = RunService.RenderStepped:Connect(function()
                if cam then cam.CFrame = fcf end
            end)
        end
    end
end)
...
addToggle(page3, layout3, "Freeze Player", function(e)
    if Player.Character then
        local root = Player.Character:FindFirstChild("HumanoidRootPart")
        if root then root.Anchored = e end
    end
end)
...
addButton(page3, layout3, "Explode", function()
    if Player.Character then
        ...
        if root then
            ...
            task.wait(0.1)
            if hum and hum.Parent then hum.Health = sh end
        end
    end
end)
...
addToggleWithInput(page3, layout3, "Anim Speed", "1", "1",
    function(e)
        if e then
            ...
            if connections.AnimSpeed then connections.AnimSpeed:Disconnect() end
        end
    end,
    function(v)
...
addToggle(page3, layout3, "Fly (Mobile)", function(e)
    ...
    if e then
        if FlyControlWindow then FlyControlWindow.Visible = true end
        if FlyButton then FlyButton.Visible = true end
        if DPad then DPad.Visible = true end
        if FlyGui then FlyGui.Enabled = true end
        if FlySpeedInput then FlySpeedInput.Text = tostring(Features.Fly.Speed) end
        if FlySpeedText then FlySpeedText.Text = tostring(Features.Fly.Speed) end
        startMobileFly()
    else
        stopMobileFly()
        if FlyControlWindow then FlyControlWindow.Visible = false end
        if FlyButton then FlyButton.Visible = false end
        if DPad then DPad.Visible = false end
...
local function togglePlayerList()
    ...
    if playerListOpen then
        ...
        plLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            if playerListScrollingFrame then
...
addToggle(page4, layout4, "Start Mimic", function(e)
    ...
    if e then
        if MimicMenuBtn then MimicMenuBtn.Visible = true end
        if MimicMenuWindow then MimicMenuWindow.Visible = true end
...
local refreshTimerConnection = RunService.Heartbeat:Connect(function(dt) 
    ...
    if refreshTimerLabel then refreshTimerLabel.Text = "⏳ Refresh time: " .. math.ceil(refreshCountdown) end
end)
...
-- ==================== TAB 5: BOT ====================
local botSpeedContainer = Instance.new("Frame")
botSpeedContainer.Size = UDim2.new(1, 0, 0, 34)
botSpeedContainer.BackgroundColor3 = Color3.fromRGB(32, 32, 46)
botSpeedContainer.BorderSizePixel = 0
botSpeedContainer.Parent = page5
addCorner(botSpeedContainer, 5)
addStroke(botSpeedContainer, 1, Color3.fromRGB(50, 50, 70))

local botSpeedLabel = Instance.new("TextLabel")
botSpeedLabel.Size = UDim2.new(0, 70, 1, 0)
botSpeedLabel.Position = UDim2.new(0, 8, 0, 0)
botSpeedLabel.BackgroundTransparency = 1
botSpeedLabel.Text = "Speed Bot"
botSpeedLabel.TextColor3 = Color3.new(1, 1, 1)
botSpeedLabel.Font = Enum.Font.Gotham
botSpeedLabel.TextSize = 10
botSpeedLabel.TextXAlignment = Enum.TextXAlignment.Left
botSpeedLabel.Parent = botSpeedContainer

local botSpeedInputBg = Instance.new("Frame")
botSpeedInputBg.Size = UDim2.new(0, 40, 0, 20)
botSpeedInputBg.Position = UDim2.new(0, 80, 0.5, -10)
botSpeedInputBg.BackgroundColor3 = Color3.fromRGB(22, 22, 34)
botSpeedInputBg.BorderSizePixel = 0
botSpeedInputBg.Parent = botSpeedContainer
addCorner(botSpeedInputBg, 3)

local botSpeedInput = Instance.new("TextBox")
botSpeedInput.Size = UDim2.new(1, -6, 1, 0)
botSpeedInput.Position = UDim2.new(0, 3, 0, 0)
botSpeedInput.BackgroundTransparency = 1
botSpeedInput.PlaceholderText = "16"
botSpeedInput.PlaceholderColor3 = Color3.fromRGB(100, 100, 120)
botSpeedInput.Text = "16"
botSpeedInput.TextColor3 = Color3.new(1, 1, 1)
botSpeedInput.Font = Enum.Font.Gotham
botSpeedInput.TextSize = 10
botSpeedInput.Parent = botSpeedInputBg

local botSpeedToggleBg = Instance.new("Frame")
botSpeedToggleBg.Size = UDim2.new(0, 32, 0, 16)
botSpeedToggleBg.Position = UDim2.new(1, -42, 0.5, -8)
botSpeedToggleBg.BackgroundColor3 = Color3.fromRGB(22, 22, 34)
botSpeedToggleBg.BorderSizePixel = 0
botSpeedToggleBg.Parent = botSpeedContainer
addCorner(botSpeedToggleBg, 1)

local botSpeedDot = Instance.new("Frame")
botSpeedDot.Size = UDim2.new(0, 12, 0, 12)
botSpeedDot.Position = UDim2.new(0, 2, 0.5, -6)
botSpeedDot.BackgroundColor3 = Color3.fromRGB(180, 180, 200)
botSpeedDot.BorderSizePixel = 0
botSpeedDot.Parent = botSpeedToggleBg
addCorner(botSpeedDot, 1)

local botSpeedClick = Instance.new("TextButton")
botSpeedClick.Size = UDim2.new(0, 40, 1, 0)
botSpeedClick.Position = UDim2.new(1, -44, 0, 0)
botSpeedClick.BackgroundTransparency = 1
botSpeedClick.Text = ""
botSpeedClick.ZIndex = 5
botSpeedClick.Parent = botSpeedContainer
...
BotEditorWindow.Size = UDim2.new(0, 280, 0, 260)
BotEditorWindow.Position = UDim2.new(0.7, -140, 0.3, -130)
...
beClose.Activated:Connect(function() if BotEditorWindow then BotEditorWindow.Visible = false end end)
...
addBEBtn("🗑 Delete Last Point", function()
    ...
    local lastWp = Features.Bot.Waypoints[#Features.Bot.Waypoints]
    if lastWp then
        if lastWp.Beam then destroyBeam(lastWp.Beam) end
        if lastWp.Part and lastWp.Part.Parent then lastWp.Part:Destroy() end
    end
...
botToggleBtn.Activated:Connect(function()
    ...
    if Features.Bot.Enabled then
        ...
        if botToggleStroke then botToggleStroke.Color = Color3.fromRGB(60, 140, 60) end
        removeLastWPToPlayerBeam()
        startBotMovement()
    else
        botToggleBtn.Text = "▶ Start Bot"
...
BotButton.Activated:Connect(function() if BotEditorWindow then BotEditorWindow.Visible = not BotEditorWindow.Visible end end)
...
chatDisplayFrame.Size = UDim2.new(1, -10, 0, 340)
...
inputFrame.Position = UDim2.new(0, 5, 0, 355)
...
sendBtn.Position = UDim2.new(1, -66, 0.5, -16)
...
for i = 1, 3 do
    addChatMessage("SYSTEM", "Chat system ready! Send messages below.", true)
end
