--[[
    WeteroScript v11.02 FINAL FIXED v12
    Author: @WeteroScript
    ADDED: Custom R6 Animations (created via code, no IDs needed)
]]
...
local Window = Instance.new("Frame"); Window.Size = UDim2.new(0, 620, 0, 520); Window.Position = UDim2.new(0.5, -310, 0.5, -260)
...
local page6, layout6 = createTab("R6 Anim", 6)
...
local Features = {
    ...
    R6Anim = {Enabled = false, CurrentAnim = nil, CurrentStyle = nil}
}
...
-- ==================== CUSTOM R6 ANIMATIONS (CREATED VIA CODE) ====================
-- Сохраняем оригинальные CFrame частей тела
local originalPoses = {}
local animationConnections = {}
local currentAnimStyle = nil
...
-- ПОЗЫ ДЛЯ КАЖДОГО СТИЛЯ АНИМАЦИИ
local ANIM_STYLES = {
    Dog = {
        name = "🐕 DOG",
        icon = "🐕",
        speed = 24,
        walkSpeed = 24,
        description = "Бег на четвереньках, язык наружу",
        -- Функция обновления позы (time - время в секундах)
        update = function(parts, time)
            local cycle = (time * 8) % (math.pi * 2)
            local legCycle = math.sin(cycle)
            
            -- Наклон туловища вперёд
            if parts.Torso then
                parts.Torso.CFrame = parts.Torso.CFrame * CFrame.Angles(math.rad(30), 0, math.rad(math.sin(cycle) * 5))
            end
            
            -- Голова
            if parts.Head then
                parts.Head.CFrame = parts.Head.CFrame * CFrame.Angles(math.rad(10), math.rad(math.sin(cycle) * 10), 0)
            end
            
            -- Руки (передние лапы)
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(-30 + legCycle * 20), 0, 0)
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(-30 - legCycle * 20), 0, 0)
            end
            
            -- Ноги (задние лапы)
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(-20 + legCycle * 15), 0, 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(-20 - legCycle * 15), 0, 0)
            end
        end
    },
    Snake = {
        ...
        speed = 12,
        walkSpeed = 12,
        description = "Ползание по земле, извиваясь",
        update = function(parts, time)
            local wave = (time * 6) % (math.pi * 2)
            local bodyWave = math.sin(wave)
            
            -- Туловище прижато к земле, извивается
            if parts.Torso then
                parts.Torso.CFrame = parts.Torso.CFrame * CFrame.Angles(math.rad(70), math.rad(bodyWave * 30), math.rad(bodyWave * 20))
            end
            
            -- Голова низко, следит за движением
            if parts.Head then
                parts.Head.CFrame = parts.Head.CFrame * CFrame.Angles(math.rad(20), math.rad(bodyWave * 40), math.rad(bodyWave * 10))
            end
            
            -- Руки прижаты
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(80), math.rad(30), math.rad(20))
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(80), math.rad(-30), math.rad(-20))
            end
            
            -- Ноги волочатся
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(40), math.rad(bodyWave * 20), 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(40), math.rad(-bodyWave * 20), 0)
            end
        end
    },
    Cat = {
        ...
        speed = 18,
        walkSpeed = 18,
        description = "Кошачья грация, мягкие шаги",
        update = function(parts, time)
            local cycle = (time * 7) % (math.pi * 2)
            local step = math.sin(cycle)
            
            -- Туловище гибкое
            if parts.Torso then
                parts.Torso.CFrame = parts.Torso.CFrame * CFrame.Angles(math.rad(10), math.rad(math.sin(cycle) * 8), 0)
            end
            
            -- Голова поворачивается
            if parts.Head then
                parts.Head.CFrame = parts.Head.CFrame * CFrame.Angles(0, math.rad(math.sin(cycle) * 15), math.rad(5))
            end
            
            -- Лапки мягкие
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(-20 + step * 25), math.rad(10), math.rad(5))
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(-20 - step * 25), math.rad(-10), math.rad(-5))
            end
            
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(-10 + step * 20), 0, 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(-10 - step * 20), 0, 0)
            end
        end
    },
    Spider = {
        ...
        speed = 14,
        walkSpeed = 14,
        description = "Паук, 8 лап",
        update = function(parts, time)
            local cycle = (time * 9) % (math.pi * 2)
            local step = math.sin(cycle)
            
            -- Туловище низко, ноги широко
            if parts.Torso then
                parts.Torso.CFrame = parts.Torso.CFrame * CFrame.Angles(math.rad(40), 0, 0)
            end
            
            -- Руки и ноги как лапы паука
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(50 + step * 30), math.rad(-40), math.rad(-30))
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(50 - step * 30), math.rad(40), math.rad(30))
            end
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(-30 + step * 25), math.rad(-30), 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(-30 - step * 25), math.rad(30), 0)
            end
        end
    },
    Bird = {
        ...
        speed = 22,
        walkSpeed = 22,
        description = "Птица, махание крыльями",
        update = function(parts, time)
            local flap = (time * 12) % (math.pi * 2)
            local wingAngle = math.sin(flap) * 60
            
            -- Руки как крылья
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(wingAngle), math.rad(30), 0)
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(-wingAngle), math.rad(-30), 0)
            end
        end
    },
    Zombie = {
        ...
        speed = 8,
        walkSpeed = 8,
        description = "Ходячий мертвец, шатание",
        update = function(parts, time)
            local stagger = (time * 3) % (math.pi * 2)
            local shake = math.sin(stagger)
            
            -- Туловище наклонено
            if parts.Torso then
                parts.Torso.CFrame = parts.Torso.CFrame * CFrame.Angles(math.rad(15), math.rad(shake * 15), math.rad(shake * 10))
            end
            
            -- Голова болтается
            if parts.Head then
                parts.Head.CFrame = parts.Head.CFrame * CFrame.Angles(math.rad(shake * 20), math.rad(shake * 30), math.rad(shake * 15))
            end
            
            -- Руки вытянуты вперёд
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(-50 + shake * 10), math.rad(20), 0)
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(-50 - shake * 10), math.rad(-20), 0)
            end
            
            -- Ноги волочатся
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(10 + shake * 15), 0, 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(10 - shake * 15), 0, 0)
            end
        end
    },
    Ninja = {
        ...
        speed = 28,
        walkSpeed = 28,
        description = "Бег согнувшись, тень",
        update = function(parts, time)
            local cycle = (time * 10) % (math.pi * 2)
            local step = math.sin(cycle)
            ...
            -- Руки сзади для скорости
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(-40 + step * 35), math.rad(-30), math.rad(-10))
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(-40 - step * 35), math.rad(30), math.rad(10))
            end
            
            -- Широкие шаги
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(-30 + step * 35), math.rad(-15), 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(-30 - step * 35), math.rad(15), 0)
            end
        end
    },
    Dino = {
        ...
        speed = 16,
        walkSpeed = 16,
        description = "Динозавр, тяжёлые шаги",
        update = function(parts, time)
            local stomp = (time * 5) % (math.pi * 2)
            local step = math.sin(stomp)
            
            -- Туловище наклонено вперёд, маленькие руки
            if parts.Torso then
                parts.Torso.CFrame = parts.Torso.CFrame * CFrame.Angles(math.rad(45), 0, math.rad(step * 5))
            end
            
            -- Маленькие руки-лапки
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(20), math.rad(30), math.rad(20))
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(20), math.rad(-30), math.rad(-20))
            end
            
            -- Тяжёлые ноги
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(-15 + step * 20), math.rad(10), 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(-15 - step * 20), math.rad(-10), 0)
            end
        end
    },
    Frog = {
        ...
        speed = 15,
        walkSpeed = 15,
        description = "Лягушка, прыжки",
        update = function(parts, time)
            local hop = (time * 4) % (math.pi * 2)
            local crouch = math.abs(math.sin(hop))
            
            -- Приседание перед прыжком
            if parts.Torso then
                parts.Torso.CFrame = parts.Torso.CFrame * CFrame.Angles(math.rad(20), 0, 0)
                parts.Torso.CFrame = parts.Torso.CFrame + Vector3.new(0, -crouch * 0.5, 0)
            end
            
            -- Лапки согнуты
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(60 + crouch * 30), math.rad(20), 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(60 + crouch * 30), math.rad(-20), 0)
            end
        end
    },
    Robot = {
        ...
        speed = 10,
        walkSpeed = 10,
        description = "Робот, механические движения",
        update = function(parts, time)
            local tick = (time * 6) % 1
            local step = tick < 0.5 and 1 or -1
            
            -- Механические, дёрганые движения
            if parts.Torso then
                parts.Torso.CFrame = parts.Torso.CFrame * CFrame.Angles(math.rad(5), 0, math.rad(math.floor(time * 10) % 2 * 5))
            end
            
            -- Дёрганые руки
            if parts.LeftArm then
                parts.LeftArm.CFrame = parts.LeftArm.CFrame * CFrame.Angles(math.rad(-20 + step * 30), 0, 0)
            end
            if parts.RightArm then
                parts.RightArm.CFrame = parts.RightArm.CFrame * CFrame.Angles(math.rad(-20 - step * 30), 0, 0)
            end
            
            -- Механические ноги
            if parts.LeftLeg then
                parts.LeftLeg.CFrame = parts.LeftLeg.CFrame * CFrame.Angles(math.rad(step * 20), 0, 0)
            end
            if parts.RightLeg then
                parts.RightLeg.CFrame = parts.RightLeg.CFrame * CFrame.Angles(math.rad(-step * 20), 0, 0)
            end
        end
    }
}

-- Сохранение оригинальных CFrame частей тела
local function saveOriginalPose()
    if not Player.Character then return end
    for _, partName in pairs(R6_PARTS) do
        local part = Player.Character:FindFirstChild(partName)
        if part then
            originalPoses[partName] = part.CFrame
        end
    end
end

-- Восстановление оригинальной позы
local function restoreOriginalPose()
    if not Player.Character then return end
    for partName, originalCFrame in pairs(originalPoses) do
        local part = Player.Character:FindFirstChild(partName)
        if part then
...
-- Остановка анимации
local function stopR6Animation()
    for _, conn in pairs(animationConnections) do
        if conn then conn:Disconnect() end
    end
    animationConnections = {}
    restoreOriginalPose()
    currentAnimStyle = nil
    Features.R6Anim.Enabled = false
    Features.R6Anim.CurrentAnim = nil
    ...
    if currentAnimLabel then
        currentAnimLabel.Text = "🎬 Current: None"
    end
end

-- Запуск анимации
local function startR6Animation(styleKey)
    if not Player.Character then return false end
    
    local style = ANIM_STYLES[styleKey]
    if not style then return false end
    
    -- Останавливаем текущую анимацию
    stopR6Animation()
    
    -- Сохраняем оригинальную позу
    saveOriginalPose()
    
    currentAnimStyle = styleKey
    Features.R6Anim.Enabled = true
    Features.R6Anim.CurrentAnim = style.name
    
    -- Меняем скорость ходьбы
    local hum = Player.Character:FindFirstChild("Humanoid")
    if hum and style.walkSpeed then
        originalWalkSpeed = hum.WalkSpeed
        hum.WalkSpeed = style.walkSpeed
    end
    
    -- Запускаем цикл анимации
    local startTime = tick()
    local animConnection = RunService.RenderStepped:Connect(function()
        if not Features.R6Anim.Enabled or currentAnimStyle ~= styleKey then return end
        if not Player.Character then return end
        ...
        style.update(parts, currentTime)
    end)
    
    table.insert(animationConnections, animConnection)
    
    if currentAnimLabel then
        currentAnimLabel.Text = "🎬 Current: " .. style.name
    end
...
local function updateFlyVelocity() 
    ...
    if inputStates.Up then dir += cam.CFrame.LookVector end 
    if inputStates.Down then dir -= cam.CFrame.LookVector end 
    if inputStates.Left then dir -= cam.CFrame.RightVector end 
    if inputStates.Right then dir += cam.CFrame.RightVector end 
...
makeDraggable(BotEditorWindow, beTopBar); beClose.Activated:Connect(function() if BotEditorWindow then BotEditorWindow.Visible = false end end)
...
addBEBtn("🗑 Delete Last Point", function()
    ...
    removeLastWPToPlayerBeam()
    if #Features.Bot.Waypoints > 0 then        createLastWPToPlayerBeam()
    end
...
-- ==================== TAB 6: R6 ANIMATIONS ====================
-- Заголовок с текущей анимацией
local currentAnimLabel = Instance.new("TextLabel")
currentAnimLabel.Size = UDim2.new(1, 0, 0, 24)
currentAnimLabel.BackgroundColor3 = Color3.fromRGB(32, 32, 46)
currentAnimLabel.BorderSizePixel = 0
currentAnimLabel.Text = "🎬 Current: None"
currentAnimLabel.TextColor3 = Color3.fromRGB(100, 160, 255)
currentAnimLabel.Font = Enum.Font.GothamSemibold
currentAnimLabel.TextSize = 11
currentAnimLabel.Parent = page6
addCorner(currentAnimLabel, 5)

-- Кнопка остановки анимации
local stopAnimBtn = Instance.new("TextButton")
stopAnimBtn.Size = UDim2.new(1, 0, 0, 32)
stopAnimBtn.BackgroundColor3 = Color3.fromRGB(200, 40, 40)
stopAnimBtn.Text = "⏹ STOP ANIMATION"
stopAnimBtn.TextColor3 = Color3.new(1, 1, 1)
stopAnimBtn.Font = Enum.Font.GothamBold
stopAnimBtn.TextSize = 11
stopAnimBtn.BorderSizePixel = 0
stopAnimBtn.Parent = page6
addCorner(stopAnimBtn, 5)
stopAnimBtn.Activated:Connect(function()
    stopR6Animation()
end)

-- Создание кнопок для каждой анимации
local animStyleKeys = {"Dog", "Snake", "Cat", "Spider", "Bird", "Zombie", "Ninja", "Dino", "Frog", "Robot"}

for _, key in ipairs(animStyleKeys) do
    local style = ANIM_STYLES[key]
    if style then
        local container = Instance.new("Frame")
        container.Size = UDim2.new(1, 0, 0, 50)
        container.BackgroundColor3 = Color3.fromRGB(32, 32, 46)
        container.BorderSizePixel = 0
        container.Parent = page6
        ...
        nameLabel.Text = style.icon .. " " .. style.name
        ...
        speedLabel.Size = UDim2.new(0, 60, 0, 14)
        speedLabel.Position = UDim2.new(1, -65, 0, 22)
        speedLabel.BackgroundTransparency = 1
        speedLabel.Text = "⚡ " .. style.walkSpeed .. " SPD"
        speedLabel.TextColor3 = Color3.fromRGB(100, 200, 100)
        ...
        playBtn.BackgroundColor3 = Color3.fromRGB(60, 100, 255)
        playBtn.Text = "PLAY"
...
layout6:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() updateCanvas(page6, layout6) end)
...
Player.CharacterAdded:Connect(function(char)
    ...
    if Features.R6Anim.Enabled and Features.R6Anim.CurrentAnim then
        -- Восстанавливаем анимацию после респавна (нужно найти ключ по имени)
        for key, style in pairs(ANIM_STYLES) do
            if style.name == Features.R6Anim.CurrentAnim then
                task.wait(0.2)
                startR6Animation(key)
                break
            end
