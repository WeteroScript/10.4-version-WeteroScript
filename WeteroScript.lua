--[[
    WeteroScript v11.02 FINAL
    Исправления:
    1. Start Mimic показывает Mimic Menu и кнопку
    2. Добавлен Jerk во вкладку Different
]]
...
local function updateWaypointBeams()
    ...
    for i = 1, #Features.Bot.Waypoints - 1 do
        local wp = Features.Bot.Waypoints[i]; local nextWp = Features.Bot.Waypoints[i + 1]
        if wp and wp.Beam and wp.Beam.Parent and wp.Beam.Attachment0 and nextWp and nextWp.Part and nextWp.Part.Parent then wp.Beam.Attachment0.Parent = nextWp.Part end
    end
end
...
local function stopBot()
    ...
    if Player.Character then local hum = Player.Character:FindFirstChild("Humanoid"); if hum then hum.WalkSpeed = Features.Speed.Enabled and Features.Speed.Value or 16; hum.JumpPower = Features.Jump.Enabled and Features.Jump.Value or 50 end end
    resetWaypointColors(); attachLastBeamToPlayer()
end
...
RandomTargetBtn.Activated:Connect(function()
    ...
    if #players > 0 then Features.Mimic.Target = players[math.random(1, #players)]; updateMMMimicTarget(); updateMimicDisplay(); teleportToTarget() end
end)
...
addToggle(page4, layout4, "Start Mimic", function(e)
    ...
    if e then
        ...
        local target = Features.Mimic.Target; if not target.Character or not target.Character:FindFirstChild("HumanoidRootPart") then Features.Mimic.Enabled = false; MimicMenuBtn.Visible = false; MimicMenuWindow.Visible = false; return end
        local lastCFrame = target.Character.HumanoidRootPart.CFrame
        if Player.Character then local myRoot = Player.Character:FindFirstChild("HumanoidRootPart"); local myHum = Player.Character:FindFirstChild("Humanoid"); if myRoot and myHum then myHum.WalkSpeed = 0; myRoot.Anchored = false; local tc = target.Character.HumanoidRootPart.CFrame; myRoot.CFrame = CFrame.new(tc.Position + (-tc.LookVector * 5), tc.Position) end end
        local lastJump = 0
        mimicConnections.Main = RunService.Heartbeat:Connect(function()
            ...
            if Features.Mimic.SmoothFollow and lastCFrame then myRoot.CFrame = CFrame.new(myRoot.Position:Lerp(offPos, 0.15), tc.Position) else myRoot.CFrame = CFrame.new(offPos, tc.Position) end
            lastCFrame = tc
...
local BotEditorWindow = Instance.new("Frame"); BotEditorWindow.Size = UDim2.new(0, 280, 0, 210); BotEditorWindow.Position = UDim2.new(0.7, -140, 0.3, -105); BotEditorWindow.BackgroundColor3 = Color3.fromRGB(10, 10, 18); BotEditorWindow.BorderSizePixel = 0; BotEditorWindow.Visible = false; BotEditorWindow.Parent = BotEditorGui; addCorner(BotEditorWindow, 12); addStroke(BotEditorWindow, 2, Color3.fromRGB(60, 100, 255))
...
addBEBtn("📍 Create Waypoint", function()
    ...
    local beam = Instance.new("Beam"); beam.Attachment0 = Instance.new("Attachment"); beam.Attachment1 = Instance.new("Attachment"); beam.Attachment1.Parent = part
    if n == 1 then beam.Attachment0.Parent = root else local prevWp = Features.Bot.Waypoints[n-1]; if prevWp and prevWp.Part and prevWp.Part.Parent then beam.Attachment0.Parent = prevWp.Part else beam.Attachment0.Parent = root end end
    beam.Width0 = 0.2; beam.Width1 = 0.2; beam.Color = ColorSequence.new(Color3.fromRGB(60, 100, 255)); beam.Transparency = NumberSequence.new(0.3); beam.Parent = part
    table.insert(Features.Bot.Waypoints, {Part = part, Beam = beam, Position = pos}); table.insert(botWaypointBeams, beam); table.insert(botWaypointParts, part); updateWaypointBeams()
end)
